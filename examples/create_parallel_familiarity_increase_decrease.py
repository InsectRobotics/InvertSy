...
def main(*args):
    ...
    for filename in filenames:
        ...
        if show_this and dataset["dispositions"][-1] > 1:
            ...
            dis[model].append(np.mean(np.diff(parpre, axis=0), axis=0))
        ...
        if show_this and dataset["scans"][-1] > 1:
            model = dataset["model"][-1]
            rot[model].append(circmean(np.diff(rotpre, axis=0), axis=0))
        ...
        if show_this and dataset["dispositions"][-1] > 1:
            ...
            model = dataset["model"][-1]
            # d_threshold = .00
            # d_route_mean = np.absolute(d_route.mean(axis=0))
            # d_map = np.all([d_route_mean > d_threshold - 0.01, d_route_mean < d_threshold + 0.01], axis=0)
            # cert[model].append(np.mean(parpre[:, d_map], axis=1))  # familiarity

            # parpre_n = (parpre - np.nanmin(parpre) / (np.nanmax(parpre) - np.nanmin(parpre)))
            cert[model].append(np.nanstd(parpre, axis=1))  # STD
    ...
    plt.figure(f"familiarity-{'-'.join([f'{key}{value}' for key, value in show_config.items()])}",
               figsize=(8, 4))
    ...
    def kurt2cert(k):
        ...
        # return .4 / k - 3
        # return .025 / k
        # return k / (np.nanmax(k) + np.finfo(float).eps)
        return k
    ...
    plt.plot(cert_x, cert_pm, 'grey', ls='', marker='.', markersize=.5)
    ...
    # print("PM", page_pm)
    spr_pm, p_spr_pm = spearmanr(cert_x, cert_pm, nan_policy='omit', alternative=hypothesis)
    ...
    plt.plot(cert_x, cert_wn, 'grey', ls='', marker='.', markersize=.5)
    ...
    # print("WN", page_wn)
    spr_wn, p_spr_wn = spearmanr(cert_x, cert_wn, nan_policy='omit', alternative=hypothesis)
    ...
    plt.plot(cert_x, cert_ic, 'grey', ls='', marker='.', markersize=.5)
    ...
    # print("IC", page_ic)
    spr_ic, p_spr_ic = spearmanr(cert_x, cert_ic, nan_policy='omit', alternative=hypothesis)
