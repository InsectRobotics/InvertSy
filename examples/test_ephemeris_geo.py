...
labels = []
azs, alts = [], []
plt.figure("sun-course", figsize=(7, 5))
for obs in [p1, p2, p3, p4]:
    y, m, d = obs.date.year, obs.date.month, obs.date.day
    ...
    # plt.plot(az, alt, '.-', label="%s (%s)" % (obs.city, obs.date.strftime("%d/%m/%Y")))
    # plt.plot(az[24], alt[24], 'ro')
azs = np.array(azs)
alts = np.array(alts)
max_alt = alts.max()
